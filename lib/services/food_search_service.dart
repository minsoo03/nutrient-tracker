...
import 'package:nutrient_tracker/services/food_standardizer.dart';
...
class FoodSearchService {
  ...
  Future<List<FoodModel>> searchFoods(String query) async {
    ...
    if (_containsKorean(normalized)) {
      return _searchKorean(normalized);
    }
    final usdaResults = await _searchUsda(normalized);
    return FoodStandardizer.prepend(normalized, usdaResults);
  }

  Future<List<FoodModel>> _searchKorean(String query) async {
    final standardized = FoodStandardizer.prepend(query, const []);
    ...
    for (final variant in _buildKoreanQueryVariants(query)) {
      ...
      for (final food in _rankFoods(results, query)) {
        ...
        if (seen.add(key)) merged.add(food);
      }
      if (merged.length > standardized.length) return merged;
    }

    for (final alias in _buildEnglishAliases(query)) {
      final results = await _searchUsda(alias);
      for (final food in _rankFoods(
        FoodStandardizer.prepend(query, results),
        alias,
      )) {
        final key = '${food.source}:${food.name}';
        if (seen.add(key)) merged.add(food);
  ...
  List<String> _buildKoreanQueryVariants(String query) {
    ...
    final unique = <String>{};
    return [collapsed, compact, cleaned, ...tokens]
        .where((v) => v.isNotEmpty && unique.add(v))
        .toList();
  }

  List<String> _buildEnglishAliases(String query) {
    final aliases = <String>[];
    for (final entry in koreanToEnglishAliases.entries) {
      if (query.contains(entry.key)) aliases.addAll(entry.value);
    }
